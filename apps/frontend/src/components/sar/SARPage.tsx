...
import type { SARFiveWOneH } from "../../types/sar";
...
export function SARPage() {
  ...
  const updateFiveWOneH = (field: keyof SARFiveWOneH, value: string) =>
    setFiveWOneH((current) => ({ ...current, [field]: value }));
  const saveDraft = () =>
    setNotice(
    ...
    <div className="page sar-page">
      ...
      <header className="sar-header">
        ...
        <div className="sar-header-actions">
          <span className="sar-pending">
            <LockKeyhole size={14} /> Generation endpoint pending
          </span>
          <button
        ...
        <main className="sar-main">
          ...
          <section className="card sar-card">
            <SARCardTitle
              icon={FileText}
              title="SAR narrative"
              caption="Editable workspace. No narrative is generated or submitted until a backend contract exists."
            />
            <textarea
          ...
          <section className="card sar-card">
            <SARCardTitle
              ...
              caption="Only evidence returned by the backend can be cited here."
            />
            <div className="sar-empty-panel">
              <CircleHelp size={20} />
              <strong>No SAR evidence references available</strong>
              <span>
                The current case response does not include SAR evidence IDs or
                source references.
              </span>
            </div>
          </section>
        ...
        <aside className="sar-side">
          ...
          <section className="card sar-card">
            <SARCardTitle
              icon={CircleHelp}
              title="Validation"
              caption="Future SAR validator output"
            />
            <div className="validation-pending">
              ...
              <p>
                Validation will appear when a SAR generation and validation
                endpoint is available.
              </p>
              <span className="validation-row">
                5W1H completeness <b>Pending</b>
              </span>
              <span className="validation-row">
                Factual consistency <b>Pending</b>
              </span>
              <span className="validation-row">
                Evidence coverage <b>Pending</b>
              </span>
            </div>
          </section>
          <section className="card sar-card">
...
function StatusBadge({ value }: { value: string | null | undefined }) {
  return (
    <span
      className={`badge status-badge ${(value ?? "").toLowerCase().replace("_", "-")}`}
    >
      <i />
      {titleCase(value)}
    </span>
